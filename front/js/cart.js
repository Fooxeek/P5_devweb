...
const displayProducts = () => {
  ...
  if (cart.length === 0) {
    ...
    for (let i = 0; i < cart.length; i++) {
      fetch("https://seckanap.loganben.com:3010/api/products/" + cart[i].id)
        .then((canape) => {
          return canape.json();
        })
        //On crée le produit en y ajoutant les éléments modifications
        .then((product) => {
          const modifications = createCart(
            product,
            cart[i].quantity,
            cart[i].color
          );
...
const removeItem = (e) => {
  ...
  newCart = cart.filter(
    (item) =>
      item.id !== idItem || (item.id === idItem && item.color !== colorItem)
  );
  localStorage.setItem("panier", JSON.stringify(newCart));
...
const validAddress = (inputAddress) => {
  let addressRegExp = new RegExp(
    "^[0-9a-zA-Z]{2,}[a-z0-9 .'-]+[0-9a-zA-Z]{2,}$",
    "i"
  );
...
const postApi = (command) => {
  ...
  fetch("https://seckanap.loganben.com:3010/api/products/order", {
    method: "POST",
    headers: {
